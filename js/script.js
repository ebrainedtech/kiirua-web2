(function($) {
	...
	//Hide Loading Box (preloader1)
	function handlepreloader1() {
		if($('.preloader1').length){
			$('.preloader1').delay(200).fadeOut(500);
		}
	}
	...
	function customTabProductPageTab () {
	    if ($('.product-details-tab-title').length) {
	        ...
	        var tKIIRUAlicker = $('.product-details-tab-title ul li');
	        
	        tabWrap.children('div').hide();
	        tabWrap.children('div').eq(0).show();
	        tKIIRUAlicker.on('click', function() {
	            var tabName = $(this).data('tab-name');
	            tKIIRUAlicker.removeClass('active');
	            $(this).addClass('active');
	            var id = '#'+ tabName;
